...
[ ! -x "$(which dialog)" ] && sudo apt install dialog 1> /dev/null 2>&1 #verifica se dialog esta instalado, senão instala
# ------------------------------------------------------------------------ #
...
do
  ...
  case $acao in
    ...
    inserir)
      ...
      #stout é para escrever na caixa
    ...
    remover)
      ...
      [ $? -ne 0 ] && continue #verifica se o usuario clicou em cancelar
